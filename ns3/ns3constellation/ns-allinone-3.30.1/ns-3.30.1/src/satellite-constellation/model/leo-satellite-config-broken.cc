...
LeoSatelliteConfig::LeoSatelliteConfig (uint32_t num_planes, uint32_t num_satellites_per_plane, double altitude)
{
  ...
  for (uint32_t i=0; i<num_planes; i++)
  {
     ...
     for(uint32_t j=0; j<num_satellites_per_plane/2; j++)
     {
       ...
       oss  << Simulator::Now().GetSeconds() << ": plane # "<< i << " node # " <<j<< ": x = " << pos.x << ", y = " << pos.y << ", z = " << pos.z << std::endl;
     ...
     for(uint32_t j=num_satellites_per_plane/2; j> 0; j--)
     {
       ...
       oss  << Simulator::Now().GetSeconds() << ": plane # "<< i << " node # " <<num_satellites_per_plane - j<< ": x = " << pos.x << ", y = " << pos.y << ", z = " << pos.z << std::endl;
  ...
  oss <<"Setting up intra-plane links with distance of "<<distance<<" km and delay of "<<delay<<" seconds."<<std::endl;
  std::string info_message = oss.str();

  NS_LOG_INFO(info_message);
  ...
  for (uint32_t i=0; i<num_planes; i++)
  {
    for (uint32_t j=0; j<num_satellites_per_plane; j++)
    {
      ...
      oss <<"Plane "<<i<<": channel between node "<<j<<" and node "<<(j+1)%num_satellites_per_plane<<std::endl;
      std::string info_message = oss.str();
  ...
  oss <<"Setting up inter-plane links"<<std::endl;
  std::string info_message = oss.str();
  ...
  for (uint32_t i=0; i<num_planes; i++)
  {
    for (uint32_t j=0; j<num_satellites_per_plane; j++)
    {
      ...
      oss <<"Channel open between plane "<<i<<" satellite "<<j<<" and plane "<<(i+1)%num_planes<<" satellite "<<nodeBIndex<< " with distance "<<distance<< "km and delay of "<<delay<<" seconds"<<std::endl;
      std::string info_message = oss.str();
  ...
  oss  << "Setting up two ground stations" << std::endl;
  ...
  for (int j = 0; j<2; j++)
  {
    ...
    oss  << Simulator::Now().GetSeconds() << ": ground station # " << j << ": x = " << temp.x << ", y = " << temp.y << std::endl;
  ...
  oss <<"Setting links between ground stations and satellites"<<std::endl;
  std::string info_message = oss.str();
  ...
  for (uint32_t i=0; i<2; i++)
  {
    ...
    oss <<"Channel open between ground station " << i << " and plane " << planeIndex << " satellite "<<closestAdjSat<<" with distance "<<closestAdjSatDist<< "km and delay of "<<delay<<" seconds"<<std::endl;
    std::string info_message = oss.str();
  ...
  oss <<"Populating Routing Tables"<<std::endl;
  std::string info_message = oss.str();
  ...
  oss <<"Finished Populating Routing Tables"<<std::endl;
  std::string info_message = oss.str();
...
void LeoSatelliteConfig::UpdateLinks()
{
  ...
  oss <<std::endl<<std::endl<<std::endl<<"Updating Links"<<std::endl;
  std::string info_message = oss.str();
  ...
  for (uint32_t i=0; i<this->num_planes; i++)
  {
    ...
    for (uint32_t j=0; j<this->num_satellites_per_plane; j++)
    {
      ...
      if(currAdjNodeID == nextAdjNodeID)
      {
        ...
        oss <<"Channel updated between plane "<<i<<" satellite "<<j<<" and plane "<<(i+1)%num_planes<<" satellite "<<nextAdjNodeID<< " with distance "<<nextAdjNodeDist<< "km and delay of "<<new_delay<<" seconds"<<std::endl;
        std::string info_message = oss.str();
      ...
      else
      {
        ...
        oss <<"New channel between plane "<<i<<" satellite "<<j<<" and plane "<<(i+1)%num_planes<<" satellite "<<nextAdjNodeID<< " with distance "<<nextAdjNodeDist<< "km and delay of "<<new_delay<<" seconds"<<std::endl;
        std::string info_message = oss.str();
  ...
  for (uint32_t i=0; i<2; i++)
  {
    ...
    if(currAdjNodeID == closestAdjSat)
    {
      ...
      oss <<"Channel updated between ground station "<<i<<" and plane "<<planeIndex<<" satellite "<<closestAdjSat<< " with distance "<<closestAdjSatDist<< "km and delay of "<<new_delay<<" seconds"<<std::endl;
      std::string info_message = oss.str();
      ...
      else
      {
        ...
        oss <<"New channel between ground station "<<i<<" and plane "<<planeIndex<<" satellite "<<closestAdjSat<< " with distance "<<closestAdjSatDist<< "km and delay of "<<new_delay<<" seconds"<<std::endl;
        std::string info_message = oss.str();
  ...
  oss <<"Recomputing Routing Tables"<<std::endl;
  std::string info_message = oss.str();
  ...
  oss <<"Finished Recomputing Routing Tables"<<std::endl;
  std::string info_message = oss.str();
